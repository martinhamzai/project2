...
public class SkipList<K extends Comparable<K>, E>
    ...
    public ArrayList<KVPair<K, E>> search(K key) {
        ...
        for (int i = head.getLevel(); i >= 0; i--) { // for each level
    ...
    public void insert(KVPair<K, E> it) {
        ...
        SkipNode x = head;
        for (int i = head.getLevel(); i >= 0; i--) { // Find insert position
            while ((x.forward[i] != null) && (x.forward[i].element().key()
                .compareTo(it.key()) < 0)) {
                x = x.forward[i];
            }
     ...
     * @version 2025-02-22
     */
    private class SkipListIterator implements Iterator<KVPair<K, E>> {
